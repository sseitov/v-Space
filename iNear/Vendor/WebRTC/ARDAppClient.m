...
static RTCAudioTrack *defaultAudioTrack;
...
- (instancetype)initWithDelegate:(id<ARDAppClientDelegate>)delegate baseURL:(NSString*)baseURL
{
    if (self = [super init]) {
        _roomServerClient = [[ARDAppEngineClient alloc] initWithURL:baseURL];
        _delegate = delegate;
        NSURL *turnRequestURL = [NSURL URLWithString:kARDIceServerRequestUrl];
        _turnClient = [[ARDTURNClient alloc] initWithURL:turnRequestURL];
        [self configure];
    ...
    if (_isInitiator) {
        ...
        [_roomServerClient sendMessage:message
                                         ...
                                         NSError *error)
         {
             ...
             NSError *messageError = [[strongSelf class] errorForMessageResultType:response.result];
             if (messageError) {
...
{
    if (defaultAudioTrack == nil) {
        RTCMediaConstraints *constraints = [self defaultMediaAudioConstraints];
        RTCAudioSource *source = [_factory audioSourceWithConstraints:constraints];
        defaultAudioTrack = [_factory audioTrackWithSource:source trackId:kARDAudioTrackId];
    }
    RTCRtpSender *sender = [_peerConnection senderWithKind:kRTCMediaStreamTrackKindAudio streamId:kARDMediaStreamId];
    sender.track = defaultAudioTrack;
    return sender;
}
...
{
    NSString* offerVideo = _isAudioOnly ? @"false" : @"true";
...
+ (void)muteAudioIn:(BOOL)mute
{
    if (defaultAudioTrack) {
        [defaultAudioTrack setIsEnabled:!mute];
    }
}
