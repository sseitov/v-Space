...
@interface PostRequester () <NSURLSessionDelegate>

@property (strong, nonatomic) NSURLSession* session;
@property (strong, nonatomic) NSData *localCertData;

@end

@implementation PostRequester

+ (instancetype)shared
{
    static PostRequester *sharedRequester = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedRequester = [[self alloc] init];
    });
...
- (void)setupSertificate:(NSData*)certData
{
    _localCertData = certData;
}
...
- (void)URLSession:(NSURLSession *)session didReceiveChallenge:(NSURLAuthenticationChallenge *)challenge completionHandler:(void (^)(NSURLSessionAuthChallengeDisposition, NSURLCredential *))completionHandler
{
    if([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
        ...
        if ([remoteCertificateData isEqualToData:_certificate]) {
